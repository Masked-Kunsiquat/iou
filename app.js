...
import { generatePersonId, pickContact, formatPhone, getLast4 } from './contact-helper.js';
  ...
  return `
    <div class="card" data-id="${transaction.id}">
      ...
      <div class="flex gap-2 mt-2">
        <button class="btn btn-secondary text-sm" data-action="payment" data-id="${transaction.id}">
          Add Payment
        </button>
        <button class="btn btn-secondary text-sm" data-action="details" data-id="${transaction.id}">
          Details
        </button>
        <button class="btn btn-secondary text-sm" data-action="edit" data-id="${transaction.id}">
          Edit
        </button>
        <button class="btn btn-secondary text-sm text-red" data-action="delete" data-id="${transaction.id}">
          Delete
        </button>
      </div>
    </div>
  ...
  showModal('Record Payment', `
    <form id="paymentForm">
      <div class="mb-2">
        ...
        <span class="text-sm text-gray">Balance: ${(balance / 100).toFixed(2)}</span>
      </div>
      <div class="form-group">
        <label class="label">Amount</label>
        <input type="number" step="0.01" name="amount" class="input" 
               placeholder="0.00" required max="${balance / 100}">
      </div>
      <div class="form-group">
        <label class="label">Payment Date</label>
        <input type="date" name="paymentDate" class="input" required 
               value="${new Date().toISOString().split('T')[0]}">
      </div>
      <div class="form-group">
        <label class="label">Note (optional)</label>
        <input type="text" name="note" class="input" placeholder="Payment note">
      </div>
...
function showTransactionDetails(transaction) {
  ...
  showModal('Transaction Details', `
    <div class="mb-4">
      <strong>${person?.firstName} ${person?.lastName}</strong><br>
      <span class="text-sm text-gray">${transaction.description}</span>
    </div>
    <div class="mb-4">
      ...
      <div class="flex-between">
        <span>Status:</span>
        <span class="${transaction.status === 'paid' ? 'text-green' : ''}">${transaction.status}</span>
      </div>
    </div>
    <h3 class="font-bold mb-2">Payment History</h3>
    <div class="list">${paymentsHtml}</div>
  `);
}
...
function showEditTransactionModal(transaction) {
  ...
  const personOptions = app.persons.map(p => 
    `<option value="${p.id}" ${p.id === transaction.personId ? 'selected' : ''}>
  ...
  showModal(`Edit ${transaction.type}`, `
    <form id="editTransactionForm">
      ...
      <div class="form-group">
        <label class="label">Amount</label>
        <input type="number" step="0.01" name="amount" class="input" 
               value="${(transaction.amount / 100).toFixed(2)}" required>
      </div>
      <div class="form-group">
...
window.editPerson = function(personId) {
  ...
  showModal('Edit Person', `
    <form id="editPersonForm">
      <div class="form-group">
        ...
        <input type="text" name="firstName" class="input" value="${person.firstName}" required>
      </div>
      <div class="form-group">
        <label class="label">Last Name</label>
        <input type="text" name="lastName" class="input" value="${person.lastName || ''}">
      </div>
      <div class="form-group">
        <label class="label">Phone Number</label>
        <input type="tel" name="phone" class="input" value="${person.phone}" required>
      </div>
  ...
  document.getElementById('editPersonForm').addEventListener('submit', async (e) => {
    ...
    // Regenerate ID if name/phone changed
    const newId = await generatePersonId(person.firstName, person.phone);
    
    if (newId !== person.id) {
      // Update all transactions with new person ID
      ...
      // Save transactions with updated IDs
...
window.deletePerson = async function(personId) {
  ...
  if (hasTransactions) {
    alert('Cannot delete person with existing transactions. Delete transactions first.');
    return;
  }
...
function showPersonModal() {
  showModal('Add Person', `
    <form id="personForm">
      ...
      <div class="flex gap-2">
        ...
        <button type="button" id="pickContactBtn" class="btn btn-secondary flex-1">
          🕵️ Pick Contact
        </button>
      </div>
    </form>
  ...
  document.getElementById('pickContactBtn').addEventListener('click', async () => {
    try {
      ...
      alert('Could not pick contact: ' + err.message);
    }
  });
...
async function handleImport(e) {
  ...
  reader.onload = async (event) => {
    try {
      ...
      const shouldMerge = confirm('Merge with existing data? (Cancel to replace all)');

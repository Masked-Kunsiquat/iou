...
/**
 * Calculates the share for each participant in a split expense.
...
export function calculateShares(totalAmount, participantIds, splitType = 'equal') {
    if (splitType === 'equal') {
        const share = Math.round(totalAmount / participantIds.length);
        ...
        shares.forEach(share => {
            if (share.personId !== 'ME') {
                transactions.push({
                    ...
                    date,
                    dueDate,
                    splitId, // Link back to the original split
                    payments: [],
                    status: 'pending'
                });
            }
